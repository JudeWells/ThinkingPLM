#!/bin/bash
#SBATCH --job-name=bench_2VSM_nipah_4D5_random_update
...
cd "$WORKDIR"
python "$WORKDIR/run_profam_bagel_pipeline.py" --config "$WORKDIR/configs/pipelines/bench_2VSM_nipah_4D5_random_update.yaml"
