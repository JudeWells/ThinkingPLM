#!/bin/bash
#SBATCH --job-name=bench_4OYD_epstein_barr_4D5_profam_frozen
...
cd "$WORKDIR"
python "$WORKDIR/run_profam_bagel_pipeline.py" --config "$WORKDIR/configs/pipelines/bench_4OYD_epstein_barr_4D5_profam_frozen.yaml"
