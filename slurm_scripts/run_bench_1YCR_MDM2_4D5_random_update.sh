#!/bin/bash
#SBATCH --job-name=bench_1YCR_MDM2_4D5_random_update
...
cd "$WORKDIR"
python "$WORKDIR/run_profam_bagel_pipeline.py" --config "$WORKDIR/configs/pipelines/bench_1YCR_MDM2_4D5_random_update.yaml"
