#!/bin/bash
#SBATCH --job-name=bench_4ZQK_PD-L1_4D5_profam_update
...
cd "$WORKDIR"
python "$WORKDIR/run_profam_bagel_pipeline.py" --config "$WORKDIR/configs/pipelines/bench_4ZQK_PD-L1_4D5_profam_update.yaml"
