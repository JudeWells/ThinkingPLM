#!/bin/bash
#SBATCH --job-name=bench_2VSM_nipah_nanobody_profam_update
...
cd "$WORKDIR"
python "$WORKDIR/run_profam_bagel_pipeline.py" --config "$WORKDIR/configs/pipelines/bench_2VSM_nipah_nanobody_profam_update.yaml"
