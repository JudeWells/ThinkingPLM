#!/bin/bash
#SBATCH --job-name=bench_1TNF_TNF_alpha_4D5_profam_update
...
cd "$WORKDIR"
python "$WORKDIR/run_profam_bagel_pipeline.py" --config "$WORKDIR/configs/pipelines/bench_1TNF_TNF_alpha_4D5_profam_update.yaml"
